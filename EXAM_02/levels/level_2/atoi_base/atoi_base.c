...
int get_digit(char c, int digitsbase)
{
    if (c >= '0' && c < '0' + digitsbase) // verifica se o caractere c está entre '0' e o limite superior para dígitos numéricos na base especificada;
        ...
        return (-1); //retorna -1 se encontrar um dígito inválido, que é retornado para o loop while de ft_atoi_base e faz com que o loop encerre (foge da condição (digit = get_digit(to_lower(*str), str_base)) >= 0))
}
...
int ft_atoi_base(const char *str, int str_base)
{
    ...
    if (*str == '-') //verifica se no primeiro byte do array há um "-" -> única condição em que o número retornado deve ser negativo
    {
        sign = -1;
        ++str;
    }
